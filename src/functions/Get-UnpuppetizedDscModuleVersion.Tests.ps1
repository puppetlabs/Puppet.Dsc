    ...
    InModuleScope puppet.dsc {
      BeforeAll {
        ...
        Mock Get-PowerShellDscModule -ParameterFilter { $Name -contains 'NoSuchModule' } {
          Write-Error 'No match was found for the specified search criteria'
          return $DscModules | Where-Object -FilterScript { $_.Name -eq 'Foo' }
        }
      ...
      Context 'when no parameters are specified' {
        BeforeAll {
          $Result = Get-UnpuppetizedDscModuleVersion
        }
        ...
        It 'searches the forge for each discovered PowerShell module' {
          Should -Invoke Get-ForgeModuleInfo -Times 3 -Scope Context
        }
      ...
      Context 'with the Name parameter' {
        It 'searches the PSGallery by name' {
          { Get-UnpuppetizedDscModuleVersion -Name 'Foo' } | Should -Not -Throw
        ...
        It 'can search for multiple modules at once' -Tag 'Me' {
          { Get-UnpuppetizedDscModuleVersion -Name 'Foo', 'Bar' } | Should -Not -Throw
        ...
        It 'errors if a named module cannot be found in the PSGallery without stopping processing' {
          $Result = Get-UnpuppetizedDscModuleVersion -Name 'Foo', 'NoSuchModule' -ErrorAction SilentlyContinue -ErrorVariable TestError
          $Result.Name | Should -Be 'Foo'
          $TestError[0].Exception.Message | Should -Be 'No match was found for the specified search criteria'
        }
      }
      Context 'with the Repository parameter' {
        It 'searches the specified repository' {
          Get-UnpuppetizedDscModuleVersion -Repository 'MyRepository'
          Should -Invoke Get-PowerShellDscModule -ParameterFilter { $Repository -eq 'MyRepository' }
        }
      }
      Context 'with the ForgeUri parameter' {
        It 'passes the specified URI to Get-ForgeModuleInfo' {
          Get-UnpuppetizedDscModuleVersion -ForgeUri 'SomeUri'
          Should -Invoke Get-ForgeModuleInfo -ParameterFilter { $ForgeUri -eq 'SomeUri' }
        }
      }
      Context 'with the ForgeNameSpace parameter' {
        It 'searches the specified namespace on the Forge' {
          Get-UnpuppetizedDscModuleVersion -ForgeNameSpace 'my_company'
          Should -Invoke Get-ForgeModuleInfo -ParameterFilter { $ForgeNameSpace -eq 'my_company' }
        }
      }
      Context 'with the MinimumVersion parameter' {
        It 'returns only unpublished versions newer than the specified one across all modules' {
          $Result = Get-UnpuppetizedDscModuleVersion -MinimumVersion '2.0.0'
      ...
      Context 'with the OnlyNewer parameter' {
        It 'returns only unpublished versions newer than any existing published version on the forge' {
          $Result = Get-UnpuppetizedDscModuleVersion -OnlyNewer
          $Result.Count | Should -Be 2
          $Result[0].Name | Should -Be 'Foo'
          $Result[0].Versions | Sort-Object -Descending | Should -Be @('3.0.0.0')
          $Result[1].Name | Should -Be 'Baz'
