function Publish-PuppetModule {
  ...
  .EXAMPLE
    ...
    leveraging the forge token stored in the FORGE_TOKEN environment variable
  .EXAMPLE
    Publish-PuppetModule -Build -Publish -PuppetModuleFolderPath ./foo -ExportFolder C:\dsc
  ...
  process {
    Try {
      ...
      If ($Build) {
        ...
        If (![string]::IsNullOrEmpty($ExportFolder)) { $ExportParameters.ExportFolder = $ExportFolder }
      ...
      If ($Publish) {
        ...
        If (![string]::IsNullOrEmpty($ExportFolder)) {
          ...
          $PackagedModulePath = Get-Item -Path (Join-Path -Path $ExportFolder -ChildPath '*.tar.gz') |
