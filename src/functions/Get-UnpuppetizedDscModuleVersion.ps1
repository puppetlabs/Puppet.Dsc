Function Get-UnpuppetizedDscModuleVersion {
  ...
  Begin {
    ...
    If ($MinimumVersion) { $VersionsToReleaseFilterScript += ' -and [version]$_ -ge [version](ConvertTo-StandardizedVersionString -Version $MinimumVersion)' }
    If ($OnlyNewer) { $VersionsToReleaseFilterScript += ' -and [version]$_ -gt [version]$VersionsReleasedToForge[0]' }
    $VersionsToReleaseFilterScript = [scriptblock]::Create($VersionsToReleaseFilterScript)
